...
public class ProjectExplorer extends JTabbedPane implements Observer {
	AppModel model;
	MenuBarController mbc;// 菜单栏controller
	private JTree tree;
	DefaultMutableTreeNode root = null;
	DefaultTreeModel treeModel;
	ProjectExplorerController controller;// 项目浏览controller

	public ProjectExplorer(AppModel model) {
		this.model = model;
		this.model.addObserver(this);// 作为一个观察者，订阅appmodel这个被观察者，重写updatet方法
		initialize();
	}
		...
		for (ProjectModel project : model.getProjects()) {
			...
			for (Diagram diagram : project.getDiagrams()) {
				...
				for (int i = 0; i < diagram.getNodes().size(); i++) {
					DefaultMutableTreeNode nNode = (DefaultMutableTreeNode) find(dNode,
							diagram.getNodes().get(i).toString() + " " + diagram.getNodes().get(i).hashCode());
					if (nNode == null) {
						nNode = new DefaultMutableTreeNode(
								diagram.getNodes().get(i).toString() + " " + diagram.getNodes().get(i).hashCode());

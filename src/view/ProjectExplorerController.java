...
public class ProjectExplorerController {
	...
	public ProjectExplorerController(JTree tree, AppModel model) {
		...
		this.tree = tree;
		MyTreeCellEditor editor = new MyTreeCellEditor(tree, (CustomTreeCellRenderer) tree.getCellRenderer());
		...
		newProjectItem = new MenuItemCtl(popupMenu, "popup.new.project", bundle, "image/project_opened.gif",
				new newProject());
	...
	private static class MyTreeCellEditor extends DefaultTreeCellEditor {
		...
		public boolean isCellEditable(EventObject e) {
			return super.isCellEditable(e) && (((TreeNode) lastPath.getLastPathComponent()) instanceof DiagramNode
	...
	public class delete extends Executor {
		public void execute(ActionEvent event) {
			if (selectedDiagram != null) {
				...
				((windowCloseExecutor) MenuBarController.menuBarView.windowClose.getWmvcExecutor())
						.close(selectedDiagram.getUserObject());
			...
			if (selectedProject != null) {
				...
				for (Diagram diagram : selectedProject.getUserObject().getDiagrams()) {
					((windowCloseExecutor) MenuBarController.menuBarView.windowClose.getWmvcExecutor()).close(diagram);
				}
			...
			if (node != null)
				if (node instanceof ProjectNode) {
					...
					if (e.getClickCount() == 2) {
						fileOpenExecutor fo = ((fileOpenExecutor) (MenuBarController.menuBarView.fileOpen
								.getWmvcExecutor()));
